...
def handle_mode1():
    ...
    while True:
        ...
        try:
            ...
            if motion == "forward":
                pid_left = PID(kp_lin, ki_lin, kd_lin, setpoint=left_disp, output_limits=(0.55,0.75), starting_output=linear_speed)
                pid_right =  PID(kp_lin, ki_lin, kd_lin, setpoint=right_disp, output_limits=(0.55,0.75), starting_output=linear_speed)
                while (left_encoder.value < abs(left_disp) - linear_tolerance) and (right_encoder.value < abs(right_disp) - linear_tolerance):
                    ...
                    left_speed = pid_left(left_encoder.value) + 0.015
